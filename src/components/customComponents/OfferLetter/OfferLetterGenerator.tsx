...
//     try {
//       // Show the hidden letter div for capturing
//       letterRef.current.style.display = 'block';
      
//       // Give time for background image to load properly
//       await new Promise(resolve => setTimeout(resolve, 1000));
      
//       // Capture with optimized settings for background images
...
//         logging: false,
//         onclone: (clonedDoc) => {
//           // Find the background image in the cloned document
//           const imgElement = clonedDoc.querySelector('img');
//           if (imgElement) {
//             // Make sure image is loaded before capturing
...
//   // Optimized PDF download function with better image handling
...
//   // Fix for the duplicate variable name error
...
//           <div className="mt-2">
//             <Input 
//               placeholder="Search by name, email, institute or course..." 
//               value={searchTerm}
...
function mapSheetDataToFormData(item: any): OfferLetterFormData | null {
  ...
  return {
    ...
    address: 
      ...
      normalizedItem["address"] || 
      '',
    serialNumber: srNo || undefined // Use the Sr. No. from sheet
  };
}
...
export default function OfferLetterGenerator() {
  ...
  const [isGeneratingPdf, setIsGeneratingPdf] = useState(false);
  const [isSendingEmail, setIsSendingEmail] = useState(false);
  const [nextSerialNumber, setNextSerialNumber] = useState<number>(1000); // For manual form entries
